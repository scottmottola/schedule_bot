...
def create_discord_channel(discord_channel, discord_server, discord_user):
    ...
    for server in discordserver:
        jserver = server.json()
        if server.status_code != 200:
            if server.status_code == 404:
                server = create_discord_server(discord_server, discord_user)
...
def create_plan(plan, discord_user, discord_channel, discord_server):
    ...
    for channel in discordchannel:
        jchannel = channel.json()
        if channel.status_code != 200:
            if channel.status_code == 404:
                channel = create_discord_channel(discord_channel, discord_server, discord_user)
                "This user is not connected to this channel. Created a connection to channel. Please try again."
            else:
                return "Error with the channel"
        if jchannel["discord_user_id"] == rserver["discord_user_id"]:
            rchannel = jchannel
    ...
    for planner in planners:
        jplanner = planner.json()
        if planner.status_code != 200:
            if planner.status_code == 404:
                planner = create_planner(discord_user)
...
async def on_message(message):
    ...
    if msg.startswith('/help'):
        ...
        /add plan;(plan-name)/[plan start](2012-01-01T23:30:00)[yyyy-mm-ddThh:mm:ss]/[plan end](2012-01-01T23:30:00+02:00)[yyyy-mm-ddThh:mm:ss]/
    ...
    if msg.startswith('/add plan'):
        await message.channel.send(create_plan(msg, message.author.name, message.channel.mention, message.guild.name))
    ...
    if msg.startswith('/mute channel'):
        await message.channel.send(mute_channel(message.channel.mention))
    
    if msg.startswith('/mute me'):
        await message.channel.send(mute_me(message.author.name, message.channel.mention))
